<template>
  <div class="page">
    <div class="page-header">
      <div>
        <h2>待结账</h2>
        <p class="sub">个人工作台 - 整理、修改、确认待结账记录</p>
      </div>
      <div class="actions">
        <el-button
          type="primary"
    ...
    <el-card>
      <el-form :inline="true" :model="filters" class="filter-form">
        <!-- 待结账界面可以切换状态：DRAFT（默认）、CONFIRMED、全部 -->
...
const PAGE_SIZE_KEY = 'draft-settlements-page-size';
const savedPageSize = Number(localStorage.getItem(PAGE_SIZE_KEY)) || 50;

const filters = reactive({
  status: 'DRAFT', // 默认显示 DRAFT 状态，可以切换到 CONFIRMED 或全部
  ownerUsername: '',
  submitterUsername: '',
  trackingNumber: '',
  model: '',
...
const resetFilters = () => {
  ...
  // 清空筛选条件
  filters.status = 'DRAFT'; // 保持 DRAFT 状态
...
const handleMoveToPending = async () => {
  ...
  await ElMessageBox.confirm(`确认将选中的 ${selectedIds.value.length} 条记录移动到结账管理吗？`, '提示', {
    type: 'info',
    confirmButtonText: '确认移动',
    cancelButtonText: '取消'
  });
  const count = await moveToPending(selectedIds.value);
  ElMessage.success(`已将 ${count} 条记录移动到结账管理`);
  selectedIds.value = [];
  loadData();
};
...
const handleConfirmAll = async () => {
  ...
  loading.value = true;
  try {
    // 准备筛选参数，只确认 DRAFT 状态的记录
    const payload: SettlementFilterRequest = { ...params.value, status: 'DRAFT' };
