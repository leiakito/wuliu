    ...
    CREATE TABLE order_record (
                                  ...
                                  category        VARCHAR(64),
                                  status          VARCHAR(32),
                                  status_changed_at DATETIME      COMMENT '最后状态变更时间',
    ...
    CREATE TABLE user_submission (
                                     ...
                                     order_date       DATE            COMMENT '用户指定的订单日期，用于匹配特定日期的订单',
                                     remark           VARCHAR(255),
                                     created_at       DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
                                     updated_at       DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                                     deleted          TINYINT         NOT NULL DEFAULT 0,
                                     UNIQUE KEY uk_submission_tracking_date_active(tracking_number, order_date, deleted),
                                     INDEX idx_submission_status(status),
                                     INDEX idx_submission_date(submission_date),
                                     INDEX idx_submission_order_date(order_date),
